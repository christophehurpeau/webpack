...
const ModuleMainTemplatePlugin = require("../lib/ModuleMainTemplatePlugin");

describe("ModuleMainTemplatePlugin", () => {
	let env;
	...
	const setupPluginAndGetEventBinding = function(name) {
		const templatePlugin = applyTemplatePluginWithOptions(ModuleMainTemplatePlugin, name);
	...
	it("has apply function", () => new ModuleMainTemplatePlugin().apply.should.be.a.Function());

	describe("when applied", () => {
		beforeEach(() =>
			env.templatePlugin = applyTemplatePluginWithOptions(ModuleMainTemplatePlugin, "foo"));
			...
			describe("render-with-entry handler", () => {
				...
				describe("with name", () => {
					...
					it("creates source wrapper with module name and external dependencies", () => {
						...
						source.source().should.be.exactly("export const templateName = moduleSource()");
					});
				});
				...
				describe("with external dependencies", () => {
					...
					it("creates source wrapper with external dependencies", () => {
						const source = env.eventBinding.handler("moduleSource()", env.chunk, "bar");
						source.should.be.instanceof(ConcatSource);
						source.source().should.be.exactly("export default moduleSource()");
					});
				});
		...
		describe("main template event handlers", () => {
			...
			describe("hash handler", () => {
				...
				it("updates hash", () => {
					...
					hash.update.firstCall.args[0].should.be.exactly("exports module");
